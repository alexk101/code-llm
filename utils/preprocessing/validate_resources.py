...
class ResourceConfig:
    ...
    target: Optional[str] = (
        None  # Path within the extracted archive to use as the source
    )
    ...
    def __post_init__(self):
        ...
        if not re.match(r"^[a-zA-Z0-9\s\-_\.]+$", self.name):
            print(
                f"""Warning: Resource name '{self.name}' contains special
        ...
        if self.source is None and self.resource is None and self.cmd is None:
            raise ValueError(
                f"""Resource '{self.name}' must specify either 'source',
                'resource', or 'cmd'"""
            )
        ...
        if sum(x is not None for x in [self.source, self.resource, self.cmd]) > 1:
            raise ValueError(
                f"""Resource '{self.name}' can only have one of 'source',
                'resource', or 'cmd' specified"""
            )

        # Check if source path exists when specified
        if self.source is not None and not Path(self.source).exists():
            print(
                f"""Warning: Source path for '{self.name}' does not exist:
                {self.source}"""
            )

        # Check kind values - don't enforce a specific set since custom types
        # are allowed
        ...
        if self.get and self.resource is None:
            raise ValueError(
                f"""Resource '{self.name}' has 'get=True' but no 'resource'
                specified"""
            )
        ...
        if self.target and not isinstance(self.target, str):
            raise ValueError(
                f"Resource '{self.name}' has invalid target: {self.target}"
            )
            ...
            if "{" in self.resource and "}" in self.resource:
                ...
                for var in format_vars:
                    if var not in self.resource_args:
                        print(
                            f"""Warning: Resource '{self.name}' uses '{var}' in
                            URL but it's not in resource_args"""
                        )
            ...
            if self.file_name and "{" in self.file_name and "}" in self.file_name:
                ...
                for var in format_vars:
                    if var not in self.resource_args:
                        print(
                            f"""Warning: Resource '{self.name}' uses '{var}' in
                            file_name but it's not in resource_args"""
                        )
            ...
            if self.target and "{" in self.target and "}" in self.target:
                ...
                for var in format_vars:
                    if var not in self.resource_args:
                        print(
                            f"""Warning: Resource '{self.name}' uses '{var}' in
                            target but it's not in resource_args"""
                        )
...
def validate_resources(yaml_path: str) -> Dict[str, List[ResourceConfig]]:
    ...
    if not isinstance(raw_resources, dict):
        print("Error: Resources file must be a dictionary")
        sys.exit(1)
    ...
    if invalid_subjects:
        print(
            f"""Warning: The following subjects contain invalid characters
            for directory names: {", ".join(invalid_subjects)}"""
        )
        print(
            """Subject names should only contain letters, numbers, underscores,
            and hyphens."""
        )
        ...
        for i, resource_dict in enumerate(resources):
            try:
                ...
                if resource_path in all_resource_paths:
                    print(
                        f"""Warning: Resource '{resource.name}' in subject
                        '{subject}' has the same normalized path as another resource"""
                    )
                ...
                if subject_dir.is_dir():
                    ...
                    if subject_name not in validated_resources:
                        ...
                        resource_names = {
                            ...
                            for r in validated_resources[subject_name]
                        }
                        for resource_dir in subject_dir.glob("*"):
                            if (
                                resource_dir.is_dir()
                                and resource_dir.name not in resource_names
                            ):
                                orphaned_dirs.append(str(resource_dir))

            if orphaned_dirs:
                print(
                    f"""\nWarning: Found {len(orphaned_dirs)} orphaned resource
                    directories that are not in the YAML config:"""
                )
                for dir_path in orphaned_dirs[
                    :10
                ]:  # Show first 10 to avoid flooding output
                    print(f"  - {dir_path}")
                if len(orphaned_dirs) > 10:
                    print(f"  - ... and {len(orphaned_dirs) - 10} more")

    print(
        f"""\nResource validation successful: {len(validated_resources)} subject(s)
        with resources"""
    )
...
def check_output_structure(resources_dict: Dict[str, List[ResourceConfig]]) -> bool:
    """
    Check the output directory structure against the expected structure
    from resources.
    """
    cache_dir = Path("cache")
    md_resources_dir = cache_dir / "md_resources"
